...
func main() {
	...
	// https://localhost:2156/oauth/auth
	mux.HandleFunc("/oauth/auth", auth)
	// https://localhost:2156/oauth/token
	mux.HandleFunc("/oauth/token", token)

	mux.Handle("/service/hello", behindAuth(createTestPage("Hello World.")))
	// Won't support the case of redirects within the auth for now.
	// mux.Handle("/service/redirect1", behindAuth(createTestRedirect("https://localhost:2156/service/redirect2")))
	// mux.Handle("/service/redirect2", behindAuth(createTestRedirect("https://localhost:2157/service/redirect3")))
